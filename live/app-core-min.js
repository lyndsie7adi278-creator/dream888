/* 🛡️ ENGINE V28.0 - FULL SEPARATION STABLE */
const _styles = `
...
@keyframes gold-breath { 0% { box-shadow: 0 0 5px #fff; border-color: #fff; } 50% { box-shadow: 0 0 20px var(--g); border-color: var(--g); } 100% { box-shadow: 0 0 5px #fff; border-color: #fff; } }
@keyframes silver-breath { 0% { box-shadow: 0 0 5px #fff; border-color: #fff; } 50% { box-shadow: 0 0 15px #00bfff; border-color: #00bfff; } 100% { box-shadow: 0 0 5px #fff; border-color: #fff; } }
...
// 🏆 最終全域解鎖函數 (不使用 module 宣告，HTML 直接呼叫)
...
async function _rfQ() { if(!_u_c) return; const s = await _Db.ref('coupons/' + _u_c).get(); if(s.exists() && s.val() > 0) { _u_q = s.val(); document.getElementById('u_q').innerHTML = `可用：${_u_q} (${_u_c}) <span onclick="_sw_c()" style="cursor:pointer;color:#aaa;font-size:0.7rem;">[切換]</span>`; } else { localStorage.removeItem('_u_c'); _u_c = ""; document.getElementById('u_q').innerText = "點選卡片驗證"; } }
