/* 🛡️ SECURITY CORE V30.0 - THE FINAL STABLE SEPARATION */
const _styles = `
:root { --p: #ffb7c5; --a: #ff8fa3; --b: #fffcf9; --g: #ffd700; --dark: #2c3e50; }
...
@keyframes gold-breath { 0% { box-shadow: 0 0 5px #fff; } 50% { box-shadow: 0 0 20px var(--g); border-color: var(--g); } 100% { box-shadow: 0 0 5px #fff; } }
@keyframes silver-breath { 0% { box-shadow: 0 0 5px #fff; } 50% { box-shadow: 0 0 15px #00bfff; border-color: #00bfff; } 100% { box-shadow: 0 0 5px #fff; } }
...
.ui-c { width: 92%; max-width: 450px; background: white; border-radius: 24px; padding: 18px; margin-top: 15px; box-shadow: 0 4px 15px rgba(255,183,197,0.15); border: 1px solid #eee; margin-left: auto; margin-right: auto; }
.p_title { text-align: center; font-weight: 900; color: #ff6b81; margin-bottom: 15px; font-size: 1.1rem; }
...
.modal-box { background: white; padding: 30px; border-radius: 28px; text-align: center; width: 310px; }
...
.footer-info { margin: 20px 0 40px 0; font-size: 0.7rem; color: #b0b0b0; text-align: center; width: 100%; }
`;
...
window._v_cl = async () => { 
    ...
    if(s.exists() && s.val() > 0) { _u_c = v; localStorage.setItem('_u_c', v); document.getElementById('_m_01').style.display = 'none'; _rfQ(); if(_s_i !== null) window._ex(_s_i); } else alert("碼錯誤"); 
};
...
_histRef.on('value', (s) => { 
    const d = s.val(); _dn.clear(); if(d) { 
        ...
        document.getElementById('h_l').innerHTML = r.map(x => `<div class="h_item">👤 <b>${x.c}</b> <span>${_fm(x.g)}</span></div>`).join(''); 
    } 
});
_poolRef.on('value', (s) => { 
    const d = s.val(); if(d) { 
